...
def main() -> int:
    ...
    # 2. Get current branch name (CLI argument, git HEAD, or GITHUB_REF_NAME fallback)
    branch = ""
    ...
    if not branch or branch == "HEAD":
        print("Error: Detached HEAD state and no branch specified.", file=sys.stderr)
        return 1
    ...
    push_args = ["push", "-f", "origin", branch] + extra_args
