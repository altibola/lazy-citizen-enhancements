...
def process_language(
    ...
    try:
        ...
        def _write(p: Path, d: dict[str, str]) -> None: gen.write_ini(p, d)
...
def main(argv: list[str] | None = None) -> int:
    ...
    # ── Download mode (RSI CDN) ──────────────────────────────────────────────
    dl = parser.add_argument_group(
        "download mode",
        "Download global.ini + Game2.dcb directly from the RSI CDN. "
        "Use when you don't have a local Star Citizen installation. "
        "Mutually exclusive with --p4k / --skip-extract.",
    )
    dl.add_argument("--download", action="store_true",
                    help="Authenticate to RSI and download required files from CDN.")
    dl.add_argument("--rsi-username",
                    help="RSI account email. Defaults to RSI_USERNAME env var.")
    dl.add_argument("--rsi-password",
                    help="RSI account password. Defaults to RSI_PASSWORD env var.")
    dl.add_argument("--rsi-mfa-code",
                    help="TOTP MFA code (6 digits). If omitted you are prompted interactively.")
    dl.add_argument("--channel", default="LIVE",
                    help="Game channel to download from: LIVE (default), PTU, EPTU.")
    ...
    # ── RSI CDN download mode ────────────────────────────────────────────────
    if getattr(args, "download", False):
        import rsi_downloader

        username = args.rsi_username or os.environ.get("RSI_USERNAME", "").strip()
        password = args.rsi_password or os.environ.get("RSI_PASSWORD", "").strip()
        mfa_code = args.rsi_mfa_code or None

        if not username:
            if sys.stdin.isatty():
                username = input("RSI username (email): ").strip()
            else:
                parser.error(
                    "RSI username required — pass --rsi-username or set RSI_USERNAME env var."
                )
        if not password:
            if sys.stdin.isatty():
                password = getpass.getpass("RSI password: ").strip()
            else:
                parser.error(
                    "RSI password required — pass --rsi-password or set RSI_PASSWORD env var."
                )

        cdn_tmp = out_dir / "cdn_downloads"
        channel = getattr(args, "channel", "LIVE")

        game_version, global_ini_path, game2_dcb_path = rsi_downloader.download_pipeline_inputs(
            username=username,
            password=password,
            out_dir=cdn_tmp,
            channel=channel,
        ...
        if args.game_version:
            ...
            # Normalize RSI versionLabel to just the P4CL number so that the
            # branch name is consistent with local-extraction runs.
            # "4.8.1-live.11952564" → "11952564"
            if game_version and "." in game_version:
                tail = game_version.rsplit(".", 1)[-1]
                if tail.isdigit():
                    game_version = tail
        environment = channel.upper()

        # global.ini from CDN → base_en.ini (English base for the pipeline)
        english_base = out_dir / "base_en.ini"
        english_base.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(global_ini_path, english_base)
        logger.info(f"English global.ini → {english_base}")

        # DataForge: unforge the downloaded Game2.dcb (no unp4k needed)
        forge_dir = out_dir / "dataforge"
        if game2_dcb_path and game2_dcb_path.exists():
            unforge_exe = pak_extract.resolve_unforge_exe()
            pak_extract.extract_dataforge_from_dcb(game2_dcb_path, unforge_exe, forge_dir)
        else:
            logger.warning(
                "Game2.dcb was not downloaded — DataForge cache unavailable. "
                "Enhancement generation will be skipped."
            )

        out_dir.mkdir(parents=True, exist_ok=True)
        version_file = out_dir / "version.json"
        version_data = {"version": game_version, "environment": environment}
